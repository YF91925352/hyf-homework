...
let numbers = [1, 2, 3, 4];
let newNumbers = [];
newNumbers = numbers.filter((a) => a % 2 !== 0).map((a) => a * 2);
...
/* 1 */
const shortTitle = movies.filter((b) => b.title.length <= 15);
/* 2 */
const longTitle = movies.filter((c) => c.title.length >= 30);
/* 3 */
const moviesNumber = movies.filter(
  (d) => d.year >= 1980 && d.year <= 1989
).length;
/* 4 */
const tagMovies = movies.map((e) => {
  if (e.rating >= 7) {
    e.tag = "Good";
  } else if (e.rating < 4) {
    e.tag = "Bad";
...
/* 5 */
const chainingMovies = movies.filter((f) => f.rating > 6).map((f) => f.rating);
/* 6 */
const keywordsMovies = movies.filter((g) => {
  const arr = g.title.toLowerCase().split(" ");
  if (
    arr.includes("surfer") ||
    arr.includes("alien") ||
    arr.includes("benjamin")
  ) {
    return g;
  }
}).length;
...
/* 7 */
const duplicatedWords = movies.filter((h) => {
  const arr = h.title.toLowerCase().split(" ");
  const duplicatedWords2 = arr.filter((ele) => {
    return arr.indexOf(ele) !== arr.lastIndexOf(ele);
  });
  return duplicatedWords2.length;
});
console.log(duplicatedWords);
/* 8 */
const averageRating = (
  movies.map((j) => j.rating).reduce((A, B) => A + B) / movies.length
).toFixed(2);
console.log(averageRating);
/* 9 */
const countTagMovies = movies
  .map((k) => k.tag)
  .reduce((countTag, tag) => {
    if (tag in countTag) {
      countTag[tag]++;
